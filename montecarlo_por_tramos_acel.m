...
for k = 1:Nscans
    t = tiempos_sim(k);
    tramo_idx = find(t >= track(1).tramos_tiempos(1:end-1) & ...
                     t <  track(1).tramos_tiempos(2:end), 1);
    if isempty(tramo_idx), continue; end

    err_cv.long(tramo_idx)  = err_cv.long(tramo_idx)  + err_cv_total.long(k)/Nsim;
    err_cv.trans(tramo_idx) = err_cv.trans(tramo_idx) + err_cv_total.trans(k)/Nsim;
    err_cv.rumbo(tramo_idx) = err_cv.rumbo(tramo_idx) + err_cv_total.rumbo(k)/Nsim;
    err_cv.vel(tramo_idx)   = err_cv.vel(tramo_idx)   + err_cv_total.vel(k)/Nsim;

    err_adapt.long(tramo_idx)  = err_adapt.long(tramo_idx)  + err_adapt_total.long(k)/Nsim;
    err_adapt.trans(tramo_idx) = err_adapt.trans(tramo_idx) + err_adapt_total.trans(k)/Nsim;
    err_adapt.rumbo(tramo_idx) = err_adapt.rumbo(tramo_idx) + err_adapt_total.rumbo(k)/Nsim;
    err_adapt.vel(tramo_idx)   = err_adapt.vel(tramo_idx)   + err_adapt_total.vel(k)/Nsim;
end
